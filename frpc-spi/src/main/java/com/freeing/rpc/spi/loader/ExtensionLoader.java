...
public class ExtensionLoader<T> {
    ...
    private static final String BINGHE_DIRECTORY = "META-INF/frpc/";
    private static final String BINGHE_DIRECTORY_EXTERNAL = "META-INF/frpc/external/";
    private static final String BINGHE_DIRECTORY_INTERNAL = "META-INF/frpc/internal/";

    private static final String[] SPI_DIRECTORIES =
        new String[]{SERVICES_DIRECTORY, BINGHE_DIRECTORY, BINGHE_DIRECTORY_EXTERNAL, BINGHE_DIRECTORY_INTERNAL};
    ...
    private T createExtension(final String name) {
        ...
        if (Objects.isNull(aClass)) {
            throw new IllegalArgumentException("spi name " + "[" + name + "]" + " is error");
        }
        ...
        for (String directory : SPI_DIRECTORIES) {
            ...
            try {
                Enumeration<URL> urls = Objects.nonNull(this.classLoader) ?
                    classLoader.getResources(fileName) : ClassLoader.getSystemResources(fileName);
    ...
    private void loadClass(final Map<String, Class<?>> classes, final String name, final String classPath)
        ...
        if (Objects.isNull(oldClass)) {
            ...
            throw new IllegalStateException("load extension resources error,Duplicate class "
                + clazz.getName() + " name " + name + " on " + oldClass.getName() + " or " + subClass.getName());
        }
    }
